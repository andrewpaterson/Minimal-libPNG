...
void /* PRIVATE */
png_crc_read(png_structp png_ptr, uint8_t* buf, size_t length)
{
...
int /* PRIVATE */
png_crc_finish(png_structp png_ptr, uint32_t skip)
{
   size_t i;
...
void /* PRIVATE */
png_check_chunk_name(png_structp png_ptr, uint8_t* chunk_name)
{
...
png_do_read_interlace(png_structp png_ptr)
{
   ...
   uint32_t transformations = png_ptr->transformations;
#ifdef PNG_USE_LOCAL_ARRAYS
   ...
   if (row != NULL && row_info != NULL)
   {
      ...
      switch (row_info->pixel_depth)
      {
         case 1:
         {
            uint8_t* sp = row + (size_t)((row_info->width - 1) >> 3);
            uint8_t* dp = row + (size_t)((final_width - 1) >> 3);
            int sshift, dshift;
            int s_start, s_end, s_inc;
            ...
            {
               v = (uint8_t)((*sp >> sshift) & 0x01);
               for (j = 0; j < jstop; j++)
               {
                  *dp &= (uint8_t)((0x7f7f >> (7 - dshift)) & 0xff);
                  *dp |= (uint8_t)(v << dshift);
                  if (dshift == s_end)
                  {
                     dshift = s_start;
         ...
         case 2:
         {
            uint8_t* sp = row + (uint32_t)((row_info->width - 1) >> 2);
            uint8_t* dp = row + (uint32_t)((final_width - 1) >> 2);
            int sshift, dshift;
            int s_start, s_end, s_inc;
            int jstop = png_pass_inc[pass];
            uint32_t i;
            ...
            for (i = 0; i < row_info->width; i++)
            {
               ...
               v = (uint8_t)((*sp >> sshift) & 0x03);
               for (j = 0; j < jstop; j++)
               {
                  *dp &= (uint8_t)((0x3f3f >> (6 - dshift)) & 0xff);
                  *dp |= (uint8_t)(v << dshift);
                  if (dshift == s_end)
                  {
                     dshift = s_start;
         ...
         case 4:
         {
            uint8_t* sp = row + (size_t)((row_info->width - 1) >> 1);
            uint8_t* dp = row + (size_t)((final_width - 1) >> 1);
            int sshift, dshift;
            int s_start, s_end, s_inc;
            uint32_t i;
            int jstop = png_pass_inc[pass];
            ...
            for (i = 0; i < row_info->width; i++)
            {
               uint8_t v = (uint8_t)((*sp >> sshift) & 0xf);
               int j;

               for (j = 0; j < jstop; j++)
               {
                  *dp &= (uint8_t)((0xf0f >> (4 - dshift)) & 0xff);
                  *dp |= (uint8_t)(v << dshift);
                  if (dshift == s_end)
                  {
                     dshift = s_start;
         ...
         default:
         {
            ...
            uint8_t* sp = row + (size_t)(row_info->width - 1) * pixel_bytes;
            uint8_t* dp = row + (size_t)(final_width - 1) * pixel_bytes;
